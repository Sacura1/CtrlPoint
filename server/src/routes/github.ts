...
router.get('/auth', (req: Request, res: Response) => {
  if (!cfg.githubClientId) {
    res.status(503).send('GitHub login is not configured yet.')
    return
  }
...
router.post('/deploy-new', requireAuth, async (req: AuthRequest, res: Response, next) => {
  try {
    ...
    res.json({ siteId: site.id, mnsName })

    triggerGitHubDeploy({ ...connection, site, user }, sha).catch(err =>
      console.error(`[GitHub New Deploy] Failed for ${repoOwner}/${repoName}:`, err)
    )
  } catch (err) { next(err) }
})
...
router.post('/webhook', async (req: Request, res: Response, next) => {
  try {
    ...
    triggerGitHubDeploy(connection, sha).catch(err =>
      console.error(`[GitHub Deploy] Failed for ${repoOwner}/${repoName}:`, err)
    )
  } catch (err) { next(err) }
})

async function triggerGitHubDeploy(connection: any, sha: string) {
  const { deployGitHubSite } = await import('../services/githubDeploy')
  await deployGitHubSite(connection, sha)
}
