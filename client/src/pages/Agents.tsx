...
export default function Agents() {
  return (
    <div className="min-h-dvh overflow-x-hidden bg-[#05050d] text-[#f7f3ff]">
      <header className="sticky top-0 z-30 border-b border-white/10 bg-[#05050d]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
          ...
          <nav className="flex items-center gap-2">
            <a href={`${API_URL}/api/agent/capabilities`} target="_blank" rel="noopener noreferrer"
              className="hidden rounded-xl px-3 py-2 text-sm font-semibold text-[#b9b2d7] transition hover:text-white sm:inline-flex">
              Capabilities
            </a>
            <Link to="/auth" className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm font-semibold text-[#d9d2f0] transition hover:bg-white/[0.08] sm:px-4">
              Open app
            </Link>
          </nav>
        </div>
      ...
      <main>
        <section className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 sm:py-14 lg:grid-cols-[1fr_0.9fr] lg:items-center">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-base leading-7 text-[#aaa3c7] sm:text-lg">
              CtrlPoint gives agents an x402-paid API for publishing HTML or framework apps to Massa DeWeb. No CtrlPoint signup, no checkout page, and no Massa wallet setup for the agent.
            </p>

            <div className="mt-7 grid max-w-xl grid-cols-3 gap-2">
              <LogoTile src="/circle.svg" alt="Circle" width={112} height={29} />
              <LogoTile src="/arc.svg" alt="Arc" width={36} height={38} />
              <LogoTile src="/massa-white.svg" alt="Massa" width={124} height={24} />
            </div>

            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <a href={`${API_URL}/api/agent/manifest`} target="_blank" rel="noopener noreferrer"
                className="rounded-2xl bg-[#8b5cf6] px-5 py-3 text-center text-sm font-bold text-white shadow-[0_16px_40px_rgba(139,92,246,0.25)] transition hover:bg-[#7c3aed]">
                Agent manifest
              </a>
              <a href={`${API_URL}/api/agent/openapi.json`} target="_blank" rel="noopener noreferrer"
                className="rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-3 text-center text-sm font-bold text-[#d9d2f0] transition hover:bg-white/[0.08]">
                OpenAPI schema
              </a>
            </div>
          </div>

          <div className="min-w-0 rounded-[8px] border border-white/10 bg-[#0c0b14] p-4 shadow-2xl shadow-black/30">
            <div className="mb-4 flex items-center justify-between gap-3 border-b border-white/10 pb-3">
              <span className="text-sm font-bold text-[#d9d2f0]">Paid deploy request</span>
              <span className="shrink-0 rounded-full bg-emerald-400/10 px-2.5 py-1 text-xs font-bold text-emerald-300">x402</span>
            </div>
            <pre className="max-w-full overflow-x-auto whitespace-pre text-[11px] leading-5 text-[#cfc8ef] sm:text-xs">
        ...
        <section className="border-y border-white/10 bg-white/[0.025]">
          <div className="mx-auto grid max-w-6xl gap-3 px-4 py-8 sm:grid-cols-2 sm:px-6 lg:grid-cols-4">
            {STEPS.map(([title, text]) => (
              <div key={title} className="rounded-[8px] border border-white/10 bg-[#0a0912] p-4">
        ...
        <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
          ...
          <div className="grid gap-3 lg:grid-cols-3">
            {ROLES.map(([name, role, text]) => (
              <div key={name} className="rounded-[8px] border border-white/10 bg-[#0a0912] p-5">
                <div className="mb-4 flex h-10 items-center">
                  {name === 'Circle' && <LogoImage src="/circle.svg" alt="Circle" width={126} height={32} />}
                  {name === 'Arc' && <LogoImage src="/arc.svg" alt="Arc" width={38} height={40} />}
                  {name === 'Massa' && <LogoImage src="/massa-white.svg" alt="Massa" width={132} height={25} />}
                </div>
                <h3 className="text-lg font-black">{role}</h3>
                <p className="mt-3 text-sm leading-6 text-[#9d95ba]">{text}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-4 pb-12 sm:px-6">
          <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#9d95ba] sm:text-base">
                Deploys are priced for abuse control. Updates are much cheaper for iterative agents.
              </p>
            </div>
            <a href={`${API_URL}/api/agent/capabilities`} target="_blank" rel="noopener noreferrer"
              className="w-fit rounded-xl border border-white/10 px-4 py-2 text-sm font-bold text-[#c4b5fd] transition hover:bg-white/[0.06]">
              JSON capabilities
            </a>
          </div>
          ...
          <div className="overflow-hidden rounded-[8px] border border-white/10">
            {ENDPOINTS.map(([label, endpoint, price, text]) => (
              <div key={endpoint} className="grid gap-2 border-b border-white/10 bg-[#0a0912] p-4 last:border-b-0 md:grid-cols-[0.7fr_1.15fr_0.35fr_1fr] md:items-center">
                <span className="text-sm font-black text-white">{label}</span>
                <code className="break-all text-xs font-bold text-[#d9d2f0] sm:text-sm">{endpoint}</code>
                <span className="w-fit rounded-full bg-[#8b5cf6]/15 px-3 py-1 text-xs font-black text-[#c4b5fd]">{price}</span>
                <p className="text-sm leading-6 text-[#9d95ba]">{text}</p>
              </div>
            ))}
          </div>
        </section>
...
function LogoImage({ src, alt, width, height }: { src: string; alt: string; width: number; height: number }) {
  return (
...
function InfoPanel({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-[8px] border border-white/10 bg-[#0a0912] p-5 sm:p-6">
      <h2 className="text-xl font-black sm:text-2xl">{title}</h2>
      <p className="mt-3 text-sm leading-7 text-[#9d95ba] sm:text-base">{children}</p>
    </div>
  )
}
